...
buffer_t *buffer_create(size_t buf_size, buffer_fill_callback_t callback, void *context) {
    ...
    buffer_t *buffer = (buffer_t *)log_malloc(TAG, "buffer_t", sizeof(buffer_t));
    if (buffer == NULL) {
        return NULL; // Memory allocation failed
    }

    // Allocate memory for the data buffer
    buffer->data = (uint8_t *)log_malloc(TAG, "buffer_t.data", buf_size);
    if (buffer->data == NULL) {
        log_free(TAG, buffer); // Free the allocated buffer structure
        return NULL; // Memory allocation failed
    }
