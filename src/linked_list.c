...
#define TAG "LINKED_LIST"

// Create a new linked list
linked_list_t *linked_list_create(void) {
    linked_list_t *list = (linked_list_t *)log_malloc(TAG, "linked_list_t", sizeof(linked_list_t));
    if (list) {
        list->head = NULL;
        list->tail = NULL;
        list->size = 0;
...
void linked_list_shift(linked_list_t *list, void *data) {
    if (!list) return;
    linked_list_node_t *node = (linked_list_node_t *)log_malloc(TAG, "linked_list_node_t", sizeof(linked_list_node_t));
    if (!node) return;
    node->data = data;
    node->next = list->head;
    list->head = node;
